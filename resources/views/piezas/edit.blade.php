...
@section('content')
    ...
    <form action="{{ route('piezas.update', $pieza) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('nombre') is-invalid @enderror" id="nombre" name="nombre" value="{{ old('nombre', $pieza->nombre) }}">
            @error('nombre')
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control @error('descripcion') is-invalid @enderror" id="descripcion" name="descripcion">{{ old('descripcion', $pieza->descripcion) }}</textarea>
            @error('descripcion')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <select class="form-control @error('yacimiento_id') is-invalid @enderror" id="yacimiento_id" name="yacimiento_id">
                ...
                @foreach ($yacimientos as $yacimiento)
                    <option value="{{ $yacimiento->id }}" {{ old('yacimiento_id', $pieza->yacimiento_id) == $yacimiento->id ? 'selected' : '' }}>
                        {{ $yacimiento->nombre }}
                    </option>
                @endforeach
            </select>
